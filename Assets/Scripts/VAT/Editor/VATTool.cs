...
public class VATTool : EditorWindow
{
    ...
    private const string BASE_PATH = "Assets/Contents/VAT/";
    ...
    public System.Collections.IEnumerator GenerateTexture()
    {
        ...
        try
        {
            ...
            textureWidth = Mathf.CeilToInt(clip.length * minSamplingRate);
            if (powerOfTwo)
            ...
            if (textureWidth > MAX_TEXTURE_SIZE || textureHeight > MAX_TEXTURE_SIZE)
            {
                EditorUtility.DisplayDialog("Error",
                    string.Format("Animation clip is too long to be sampled at {0}FPS for a max texture size of {1}!",
                        minSamplingRate, MAX_TEXTURE_SIZE), "OK");
                throw new Exception();
            }
            ...
            Mesh
                bakedMesh =
                    new Mesh(); //we need to bake the skinned mesh to a regular mesh in order to get its vertex positions on each frame
            List<Vector3> tmpVPos = new List<Vector3>(); //tmp list to store the vertex positions of the baked mesh
            bounds = new Vector2(float.PositiveInfinity,
            ...
            path = EditorUtility.SaveFilePanelInProject("Save Texture", "VATTexture_" + clip.name, "png",
                "Select destination");
